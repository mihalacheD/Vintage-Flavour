      ...
      {recipe.imageUrl && (
        <div className="relative w-full h-96 bg-gray-200 overflow-hidden">
          ...
          <div className="absolute top-6 left-6">
            <span className="bg-[#79c141] text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg">
              {recipe.categories}
            </span>
          </div>
        </div>
      ...
      <div className="p-8 md:p-12">
        ...
        <div className="text-center mb-10">
          ...
          {recipeUserName?.assignedToUser && (
            <Blockquote size="3" className="text-gray-600">
              by <Em className="orange font-semibold">{recipeUserName.assignedToUser.name}</Em>
            </Blockquote>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          ...
          <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-4 border border-orange-100">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-[#ec9131] rounded-lg flex items-center justify-center text-white">
                <GiMeal size={20} />
              </div>
              <div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-100">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-[#79c141] rounded-lg flex items-center justify-center text-white">
                <MdStars size={20} />
              </div>
              <div>
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            <div className="h-1 w-12 bg-gradient-to-r from-[#ec9131] to-[#79c141] rounded-full"></div>
            ...
            <ul className="space-y-3">
              {recipe.ingredients.split("\n").map((ingredient, index) => (
                <li key={index} className="flex items-start gap-3">
                  <span className="w-6 h-6 bg-[#79c141] text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                    ✓
                  </span>
        ...
        <div>
          ...
          <div className="space-y-4">
            {recipe.instructions.split("\n").map((instruction, index) => (
              <div
                key={index}
                className="bg-white rounded-2xl p-6 border-l-4 border-[#79c141] shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="flex gap-4">
                  <div className="w-10 h-10 bg-gradient-to-br from-[#79c141] to-[#5ea330] text-white rounded-full flex items-center justify-center font-bold text-lg flex-shrink-0">
                    {index + 1}
                  </div>
